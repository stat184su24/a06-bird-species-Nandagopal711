...
* Slate-colored Junco is primarily present in one or two seasons(Winter) until March.
* Catbirds, Chipping Sparrows, House Wren, Myrtle Warbler, and Robins are all seen only in the summer months(one season).
* Song sparrows are there from March through October.
...
* Catbird - May
* Chipping Sparrow - September
* House Wren - July
* Myrtle Warbler - October
* Robin - May
* Slate-colored Junco - October
* Song Sparrow - April
* White-breasted Nuthatch - October


Double checker : 
```{r}

species_name <- "White-breasted Nuthatch"
species_data <- Joined %>%
  filter(SpeciesName == species_name)
species_monthly_count <- species_data %>%
  group_by(Month) %>%
  summarise(count = n()) %>%
  ungroup()
highest_occurring_month <- species_monthly_count %>%
  filter(count == max(count))
print(highest_occurring_month)
```
...
```{r}
species_months_count <- ByMonth %>%
  group_by(SpeciesName) %>%
  summarise(months_present = n_distinct(Month)) %>%
  filter(months_present >= 6)

print(species_months_count)
```
